...
Boucle de feedback **app → Atelier** : quand un chat Claude Code d'app (Studio) bute sur une friction **plateforme** (tool MCP, doc, build/deploy, dataverse, agent) — et **non** un bug interne de l'app — il la remonte au lieu de contourner en silence. Depuis 2026-07-06, l'axe **`kind`** distingue la nature : `error` (cassé) · `limitation` (bride) · `suggestion` (idée d'amélioration plateforme, également bienvenue). Romain consomme ces remontées via la **page `/issues`** de la homepage ou en session dev Atelier.

- **Store CENTRAL (2026-06-26)** : les remontées vivent dans le **control-plane Postgres** `atelier_meta.platform_issues` (store [crates/atelier-common/src/issue_store.rs](crates/atelier-common/src/issue_store.rs)), **plus** dans l'arbre source des apps. WHY : la feature concerne des bugs de la **plateforme**, pas des apps — le fichier per-app `{slug}/src/CLAUDE_ISSUES.json` n'avait aucun sens (il polluait le dépôt de l'app). L'ancien fichier a été **rapatrié une fois puis supprimé** au boot (`backfill_from_files`, idempotent). Le store est l'**autorité des enums** (`coerce` kind/area/severity, statuts invalides ignorés au PATCH) et **porte le sender** du canal EventBus `issue` (mirror du notification store) : chaque mutation réussie publie un `IssueEvent` (`created`/`updated` transportent l'entrée complète, `deleted` l'id — y compris la purge `delete_by_slug`), relayé en WS **`issue:event`**.
- **Endpoints** ([crates/atelier-api/src/routes/issues.rs](crates/atelier-api/src/routes/issues.rs), non authentifiés comme `build-event`/`ship`) : **report** `POST /api/apps/{slug}/issues` (`{title, kind?, area?, severity?, context?, tried?}` ; slug dans l'URL → colonne `slug` ; le serveur estampe `id`/`ts`/`status:open`) ; **triage dev platform-level** (l'`id` est global, plus de slug) : `GET /api/issues?status=&app=` (liste agrégée, tri serveur) · `PATCH /api/issues/{id}` (`status`/`note`) · `DELETE /api/issues/{id}`. Hook **AppDelete** : purge des issues de l'app (`delete_by_slug`). No-op/vide si Postgres down (mirror `TaskStore`).
- **Surface agent (2026-07-03)** : règle `.claude/rules/report-issues.md` (QUAND/quand NE PAS, barème kind/severity/area) + skill `0-report-issue` (SKILL.md seul) qui pointe vers le **tool MCP `issue_report(title, kind?, area?, severity?, context?, tried?)`** (scope projet ET whitelist surveillance — le scan-agent peut aussi remonter ; aussi dans `MCP_READONLY` du runner → utilisable en plan-mode, écrit la méta-DB seulement comme `notify_user`). L'ancien `report-issue.sh` (curl+jq) est **supprimé** et purgé des workspaces par la régénération de contexte ; l'endpoint `POST …/issues` reste (surface HTTP du même store). Régénérer le contexte des apps après tout changement (MCP `app.regenerate_context` — ou attendre le boot regen).
- **UI dédiée (2026-07-06)** : page homepage **`/issues`** ([web/src/pages/Issues.jsx](web/src/pages/Issues.jsx)) — stat tiles par kind, filtres statut/kind/app, triage (résoudre/écarter/rouvrir/note/supprimer), **rapports copiables** en markdown (par issue + « Copier tout » sur le filtre courant). Contexte [web/src/context/IssuesContext.jsx](web/src/context/IssuesContext.jsx) (fetch initial + WS `issue:event` + resync epoch, mutations optimistes — mirror NotificationsContext). **Pastille sidebar** sur l'entrée « Remontées » : rouge + count des error/limitation ouvertes ; bleue + count si uniquement des suggestions ; check vert sinon.
