...
- **Surface agent** (générée par `context.rs` dans chaque app) : règle `.claude/rules/report-issues.md` (QUAND/quand NE PAS) + skill `0-report-issue` (SKILL.md + `report-issue.sh` qui curle l'endpoint via `jq`, calqué sur `0-build`/`0-deploy`). Le script est **inchangé** (toujours `POST …/issues`) ; seule la prose précise désormais que le store est central (hors dépôt). Régénérer le contexte des apps après tout changement (MCP `app.regenerate_context`).
