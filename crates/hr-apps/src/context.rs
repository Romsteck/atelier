...
fn render_flows_first_rule(app: &Application) -> String {
    // The body contains literal `{{ ... }}` (TOML expression placeholders) so
    // we keep it as a raw string and inject `{slug}` via `replace` rather
    // than `format!`, which would force escaping every brace.
    r#"# Flows-first — doctrine d'orchestration

Cette app cible le moteur **`hr-flow`** (équivalent Power Automate, natif Rust). Toute logique métier qui chaîne plusieurs étapes doit s'exprimer comme un **flux**, pas comme du Rust écrit à la main dans un handler.

## État de la migration

Le pilote **Wallet** a validé la plateforme `hr-flow` (engine, persistence, MCP, Studio, stats). Les autres apps Rust+Vite et Rust pur sont désormais éligibles. Selon où en est cette app (`{slug}`), tu seras dans un des deux cas :

- **Pas encore intégrée à hr-flow** : `Cargo.toml` du serveur n'a pas la dep `hr-flow`, pas de dossier `flows/`, pas de `flows::build_engine()` dans `main.rs`. → Quand l'utilisateur te donnera le **prompt de migration** (il commence par "Migration vers hr-flow"), suis-le. Tu prépareras l'intégration puis migreras les routes par lot.
...
Si tu rencontres un **bug** ou une **limitation** dans `hr-flow` lui-même (engine, primitives, expressions, connecteurs managés, persistence, MCP) :
...
3. **Attends le correctif**. La plateforme `hr-flow` est maintenue dans le repo HomeRoute (`/nvme/homeroute`) par un agent dédié. Le user transmet le rapport, l'agent plateforme corrige et redéploie, puis te dit de reprendre.
...
Le code Rust de l'app reste l'**entrée du monde réel** (route HTTP, cron, webhook). Il invoque ensuite le flux : `engine.run("nom_du_flux", input).await`.

## Hiérarchie de choix

Pour chaque étape d'un flux, choisis dans cet ordre :

1. **Connecteur managé** (`dataverse`, `homeroute`, `http`) — l'option par défaut pour parler à la base, à HomeRoute ou à n'importe quelle API.
2. **Connecteur custom** (déclaré dans cette app, ex : `openrouter`) — quand un service tiers est utilisé plus d'une fois.
...
4. **Action Rust custom** (`#[flow_action]`) — **dernier recours**, uniquement pour :
   ...
   - un connecteur encore inexistant pour un service éphémère.
...
/// Apps eligible for the hr-flow rollout: any Rust backend (`axum-vite` or
/// pure `axum`). Wallet was the pilot validation ; the rule + skill ship
/// to the rest now. NextJS and Flutter remain out of scope.
fn is_flows_eligible(app: &Application) -> bool {
    matches!(app.stack, AppStack::AxumVite | AppStack::Axum)
}

fn render_flow_build_skill(_app: &Application) -> String {
    r#"---
name: flow-build
description: Crée, teste, débugge un flux hr-flow pour cette app. Utilise-moi quand l'utilisateur demande un nouveau flux, veut modifier un flux existant, ajoute un connecteur custom ou une action Rust, ou cherche pourquoi un run a échoué. Lis aussi `.claude/rules/flows-first.md` qui pose la doctrine.
allowed-tools:
---
...
Pour toute logique chaînée : **flux > primitive > action Rust**. Une action custom = une fonction pure ; toute boucle/condition/accumulation visible doit être en primitive.

## Prérequis — l'app est-elle intégrée à `hr-flow` ?

Vérifie d'abord :

1. **Dep `hr-flow` dans `server/Cargo.toml`** (ou équivalent selon la stack) — pointer relatif vers `crates/orchestrator/hr-flow/` du repo HomeRoute.
2. **Module `flows/`** dans `server/src/` qui appelle `hr_flow::FlowEngineBuilder::new()`, enregistre les connecteurs managés (`http`) et custom (`dataverse` si DV, plus les tiens), puis charge les `flows/*.toml`.
3. **`build_engine()` invoqué dans `main.rs`** et le `FlowEngine` placé dans `AppState` pour être appelé depuis les handlers.
4. **Dossier `flows/`** au niveau de `server/src/` (relatif au cwd du process).
5. **Dossier `runs/`** au niveau de `<app>/runs/` sur Medion (créé automatiquement par `JsonRunStore`, mais le `build_artefact` doit propager `flows/`).

Si **un seul** de ces points manque, l'app n'est pas intégrée. Réfère-toi au prompt de migration que t'a passé l'utilisateur ; **n'invente pas** la procédure d'intégration depuis le skill — c'est un prompt explicite et structuré.
...
1. **Définition TOML** : dans `flows/<nom>.toml`, format plat avec `parent`/`parent_branch`. Schéma minimal :
   ...
   ```
2. **Enregistrement** : dans `src/server/src/main.rs`, le moteur charge automatiquement les fichiers `flows/*.toml` au démarrage. Pas d'enregistrement manuel.
...
Si tu as besoin d'appeler un service tiers (et qu'`http` ne suffit pas pour des raisons d'auth, de format, etc.) :

1. Crée un module Rust `src/server/src/flows/connectors/<nom>.rs` qui implémente le trait `hr_flow::Connector`.
2. Dans `main.rs`, ajoute `builder.register_connector("<nom>", Arc::new(MyConnector::new(...)))`.
3. Les credentials viennent de `.env` (par app, jamais hardcodées).

### 3. Ajouter une action Rust custom (dernier recours)

1. Dans `src/server/src/flows/actions.rs` (ou un module similaire) :
   ```rust
   ...
   ```
2. Dans `main.rs`, le `register_compute_score(&mut builder)` est généré par la macro — appelle-le.
