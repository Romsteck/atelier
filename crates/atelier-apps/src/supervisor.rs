...
/// Supervises Atelier application processes via systemd transient units.
#[derive(Clone)]
pub struct AppSupervisor {
    pub registry: AppRegistry,
    pub port_registry: PortRegistry,
    processes: ProcessMap,
    app_state_tx: tokio::sync::broadcast::Sender<atelier_common::events::AppStateEvent>,
}
    ...
    async fn update_app_state(&self, slug: &str, state: AppState) {
        if let Some(mut app) = self.registry.get(slug).await {
            if app.state != state {
                ...
                let event = atelier_common::events::AppStateEvent {
                    slug: slug.to_string(),
