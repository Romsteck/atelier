...
//!
//! NB : le prompt du triage HEADLESS des remontées ne vit PAS ici mais dans
//! `atelier-pilot` (`crate::triage::build_triage_prompt`) — `atelier-api` dépend
//! d'`atelier-pilot`, pas l'inverse, et le spawn du triage est côté pilote.
...
Les remontées plateforme des agents sont triées automatiquement (une instance de triage les transforme en items backlog) : tu n'as aucune liste de remontées à lire, elles t'arrivent déjà sous forme d'items — priorise-les et affine-les comme les autres.
