//! `atelier-dataverse` — Dataverse-like managed database engine on top of
//! PostgreSQL with a dynamically-generated GraphQL surface.
//!
//! This crate is the long-term replacement for `atelier-db` (legacy SQLite). The
//! two coexist during the migration window and are selected at runtime by
//! `atelier-apps` via the `db_backend` flag on each `Application`.
//!
//! Top-level concepts:
