...
//! 2. **No SQL injection surface** — `$filter` is parsed by [`atelier_dvexpr`];
...
/// [`atelier_dvexpr::sql::Param`] but stays inside this crate because the IPC
...
pub fn build_list_sql(
    ...
    if let Some(src) = &query.filter {
        let toks = atelier_dvexpr::lexer::tokenize(src)
            .map_err(|e| DataverseError::internal(format!("$filter parse: {}", e)))?;
        let ast = atelier_dvexpr::parser::parse(&toks)
            .map_err(|e| DataverseError::internal(format!("$filter parse: {}", e)))?;
...
/// Build a [`atelier_dvexpr::checker::ColumnSchema`] over a `TableDefinition`,
