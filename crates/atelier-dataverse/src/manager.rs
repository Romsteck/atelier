...
//! file (`/var/lib/atelier/state/dataverse-secrets.json` in production), or
...
/// `atelier-dataverse` (admin + app pools). 30s is generous for OLTP — schema
/// mutations and migrations run on the admin pool which is fine, large
/// migrations should use the offline `atelier-dataverse-migrate` tool, not the
...
mod tests {
    ...
    fn unique_tmp(name: &str) -> PathBuf {
        let nanos = std::time::SystemTime::now()
            ...
            .map(|d| d.as_nanos())
            .unwrap_or(0);
        std::env::temp_dir().join(format!("atelier-dataverse-test-{}-{}.json", name, nanos))
    }
