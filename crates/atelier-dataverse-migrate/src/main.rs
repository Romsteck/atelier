//! `atelier-dataverse-migrate` — CLI thin wrapper around
//! [`atelier_dataverse_migrate::migrate_with_manager`].
//!
...
//! Usage:
//! ```text
//! atelier-dataverse-migrate \
//!   --slug wallet \
//!   --admin-url postgres://dataverse_admin:…@127.0.0.1:5432/postgres \
//!   --apps-root /var/lib/atelier/apps \
//!   [--secrets-path /var/lib/atelier/state/dataverse-secrets.json] \
...
use atelier_dataverse::{DataverseManager, ProvisioningConfig};
use atelier_dataverse_migrate::{MigrationReport, migrate_with_manager};
...
fn print_help() {
    println!(
        "atelier-dataverse-migrate — CLI ad-hoc migration tool (preferred path: agent calls db.migrate via MCP)\n\
         \n\
         Required:\n\
         ...
         \n\
         Optional:\n\
           --apps-root <path>        default /var/lib/atelier/apps\n\
...
fn print_report(opts: &CliOpts, report: &MigrationReport) {
    ...
    if opts.dry_run {
        ...
        println!("  1. Set apps[\"{}\"].db_backend = \"data-migrated\" in /opt/atelier/data/apps.json", opts.slug);
