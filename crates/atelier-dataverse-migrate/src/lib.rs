//! Library used by both the `atelier-dataverse-migrate` CLI and the
...
///   (`/var/lib/atelier/state/dataverse-secrets.json` in production).
...
async fn do_migrate(
    ...
    for table in tables {
        ...
        let id_strategy = if uuid_pk_tables.contains(&table.name) {
            atelier_dataverse::IdStrategy::Uuid
        } else {
            atelier_dataverse::IdStrategy::Bigserial
        };
...
fn read_sqlite_schema(path: &Path) -> Result<(Vec<SqliteTable>, Vec<SqliteRelation>)> {
    ...
    if !has_dv_tables {
        bail!(
            "no `_dv_tables` metadata found in {} — this app was never managed by atelier-db, \
...
fn build_table_definition(
    t: &SqliteTable,
    id_strategy: atelier_dataverse::IdStrategy,
) -> Result<atelier_dataverse::TableDefinition> {
    let now = Utc::now();
    let mut cols: Vec<atelier_dataverse::ColumnDefinition> = Vec::with_capacity(t.columns.len());
    for c in &t.columns {
        ...
        cols.push(atelier_dataverse::ColumnDefinition {
            name: c.name.clone(),
            field_type: ft,
    ...
    Ok(atelier_dataverse::TableDefinition {
        name: t.name.clone(),
        slug: t.name.clone(),
        columns: cols,
...
/// migrated with [`atelier_dataverse::IdStrategy::Uuid`] so the original
...
fn build_relation(r: &SqliteRelation) -> Result<atelier_dataverse::RelationDefinition> {
    use atelier_dataverse::{CascadeAction, CascadeRules, RelationType};
    ...
    Ok(atelier_dataverse::RelationDefinition {
