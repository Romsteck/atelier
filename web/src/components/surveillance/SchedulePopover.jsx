      ...
      {open && createPortal(
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div
            role="dialog"
            aria-modal="true"
            className="relative w-full max-w-sm rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-2xl"
          >
            <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-200 dark:border-gray-800">
              <CalendarClock className="w-4 h-4 text-emerald-600 dark:text-emerald-400 shrink-0" />
              <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100 flex-1">Surveillance automatique planifiée</h3>
              <button onClick={() => setOpen(false)} className="text-gray-400 hover:text-gray-700 dark:hover:text-gray-200" title="Fermer">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="p-4 space-y-4 text-sm">
              <label className="flex items-center gap-2 cursor-pointer select-none">
                ...
                />
                <span className="text-gray-700 dark:text-gray-300">Activer le scan planifié</span>
              </label>

              <div className={`space-y-3 ${enabled ? '' : 'opacity-50 pointer-events-none'}`}>
                <div className="flex items-center justify-between gap-2">
                  <span className="text-gray-600 dark:text-gray-400">Heure (locale)</span>
                  <select
                    value={hour}
                    onChange={(e) => onSave({ hour: Number(e.target.value) })}
                    className="bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-sm px-2 py-1 text-gray-800 dark:text-gray-200"
                  >
              ...
              <p className="text-[11px] text-gray-500 leading-relaxed border-t border-gray-200 dark:border-gray-800 pt-3">
