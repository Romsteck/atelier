...
export default function AgentPanel({ panelKey }) {
  const { slug, convos, sendMessage, answer, cancel, closeConversation } = useAgentConversations();
  ...
  return (
    <div className="flex flex-col h-full min-h-0 bg-gray-900">
      ...
      {/* Sélecteurs : modèle + mode + effort. Verrouillés pendant une session vivante
          (le runner fixe modèle/mode/effort au démarrage). Reprendront à la prochaine. */}
      <div className="flex items-center flex-wrap gap-x-4 gap-y-1 px-3 py-1.5 border-t border-gray-800 shrink-0"
        title={live ? 'Verrouillé pendant la session — Stop puis nouveau message pour changer' : undefined}>
        <div className="flex items-center gap-1">
          <span className="text-[11px] text-gray-600 mr-1">Modèle</span>
          <select value={modelId} disabled={live} onChange={(e) => onChangeModel(e.target.value)}
            className="bg-gray-800 border border-gray-700 rounded-sm text-[11px] text-gray-200 px-1 py-[3px] focus:outline-none focus:border-blue-500 disabled:opacity-50 disabled:cursor-not-allowed">
        ...
        <div className="flex items-center gap-1">
          ...
          {MODES.map((m) => (
            <button key={m.id} disabled={live} onClick={() => setMode(m.id)} title={m.title}
              className={`px-1.5 py-0.5 rounded-sm text-[11px] disabled:opacity-50 disabled:cursor-not-allowed ${
                mode === m.id
                  ? m.id === 'bypass'
