...
> Le MCP `studio` (servi par Atelier sur Medion à `http://127.0.0.1:4100/mcp`, scope par app via `?project={slug}`) est exposé dans chaque workspace d'app via `.mcp.json` (consommé par l'agent Claude) ; les tools `mcp__studio__docs_*` sont auto-approuvés en lecture. (Le port 4001 est une référence legacy hr-orchestrator morte ; aucun serveur n'écoute dessus.)
